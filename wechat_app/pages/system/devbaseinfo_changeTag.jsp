...
function checkForm(){
	if(isNull($F('tagTidNew'))){
		alert("新标签编号为空!");
		return false;
	}
	if($('tagTidNew').value == $('tagTidOld').value) {
		alert("新标签编号与旧标签相同!");
		return false;
	}
...
function vbsCall(varEpc, varTid, varUserData){
	$('tagTidNew').value = varTid.replace(/[|]/g, '');
}
...
<form name="form1" method="post" action="devbaseinfo.do?action=changeTag">
...
<table align="center" width="750" border="0" cellspacing="1" cellpadding="6" class="table1">
 <tr class="tr1">
    <td nowrap colspan="4" align="center">设备更换标签</td>
  </tr>
  <tr class="tr3">
 ...
 <tr class="tr3">
    <td nowrap><font color="red">当前标签编号：</font></td>
    <td nowrap colspan="3">
    	<input type="hidden" name="tagTidOld" value="${devbaseinfo.tagTid}" />
    	<c:out value="${devbaseinfo.tagTid}" />
    </td>
  </tr>
  <tr class="tr3">
    <td nowrap><font color="green">新标签编号：</font></td>
    <td nowrap colspan="3">
    	<input type="text" name="tagTidNew" class="paneGray" readonly />&nbsp;*
    	<input type="button" value="读取" class="button" style="width: 35px;" onclick="return readTagTid()" />&nbsp;
		<input type="button" value="重置" class="button" style="width: 35px;" onclick="javascript:$('tagTidNew').value='';" />
    </td>
  </tr>
</table>
