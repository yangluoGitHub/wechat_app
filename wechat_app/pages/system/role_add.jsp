...
<form name="form1" method="post" action="role.do?action=add">
<table align="center" width="850" border="0" cellspacing="1" cellpadding="3" class="table1">
 <tr class="tr1">
    <td nowrap colspan="2" align="center"><%=resource.srcStr("System.add_role")%></td>
  </tr> 
  <tr class="tr3">
    <td nowrap width="100"><%=resource.srcStr("System.role_name") %>：</td>
    <td nowrap><input id="roleName" type="text" style="width:153;" name="roleName" class="pane" onKeyPress="if(event.keyCode<48 || event.keyCode ==92) event.returnValue=false" maxLength="40">&nbsp;*</td> 
  </tr>  
  <!-- 
  <tr class="tr3">
    <td nowrap>角色级别：</td>
	...
	<SELECT  id="orgType" name="orgType" style="width:153;" onchange="chooseRoleButton(this.value);">
		<option value=''>-------请选择--------</option>
			<c:forEach items="${sysOrgGradeList}" var="sysOrgGrade">
				<option value='<c:out value="${sysOrgGrade.no}" />'><c:out value="${sysOrgGrade.name}角色" /></option>
			</c:forEach>
	</SELECT>&nbsp;*	
	</td>
  </tr>
   -->
  <input type="hidden" name="roleCatalog" value="3">
  <tr class="tr3">
    <td nowrap><%=resource.srcStr("System.role_remark") %>：</td>
    <td nowrap><textarea name="note" style="width:400px;height:50px;" ></textarea></td> 
  </tr>
  <tr class="tr3">
    ...
    <script type="text/javascript">
    		...
    		for(int i=0;i<menuList.size();i++){
    			...
    			for(int j=0;j<buttonList.size();j++){
    				...
    				if(aButton.getMenu().getNo().equals(aMenu.getNo())){
    					...
    					buttons = buttons + "<span style=\"border:0px;border-bottom:1px solid #cccccc;color=steelblue\"><input name=button id="+aButton.getNo()+" type=checkbox value="+aButton.getNo()+" onclick=change(this);check("+aButton.getNo().toString()+","+aMenu.getNo()+")>"+aButton.getName()+"</span>";
    	%>
    	...
    	d.add('<%= aMenu.getNo() %>','<%= aMenu.getSuperMenu().getNo() %>','<input type="checkbox" name="menu"  id="<%=aMenu.getNo() %>" onclick="check1(this);change(this);" level="<%= aMenu.getLevel() %>" value="<%= aMenu.getNo() %>" ><%= aMenu.getName() %><%= buttons %>','#','','');
...
function checkForm(){
  ...
  roleService.checkRole("",$("roleName").value,
...
//当角色级别为总行时，会显示角色增加，修改，删除权限按钮
//当角色级别为分行或者支行时，会自动隐藏角色的增加，修改，删除按钮
