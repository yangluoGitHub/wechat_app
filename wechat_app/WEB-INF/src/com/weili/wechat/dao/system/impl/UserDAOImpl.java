...
public class UserDAOImpl extends HibernateDAO implements UserDAO {
	...
	private User po_vo(OpTable aSysUser) {
		 ...
		 * 微信公众号
		 */
		aUser.setWechatId(aSysUser.getDefaultPaid());
		if (aSysUser.getDefaultPaid() != null && !aSysUser.getDefaultPaid().equals("")) {
			PublicAccount pa = (PublicAccount) findById(PublicAccount.class, aSysUser.getDefaultPaid());
			if (pa.getWechatName() != null)
				aUser.setWechatName(pa.getWechatName());
		}
		return aUser;
	}
	...
	private OpTable vo_po(User aUser) {
		 ...
		 * 微信公众号
		 */
		aSysUser.setDefaultPaid(aUser.getWechatId());
		return aSysUser;
	}
