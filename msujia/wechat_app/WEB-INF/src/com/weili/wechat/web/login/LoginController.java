...
public class LoginController extends MultiActionController {
	...
	public ModelAndView login(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (this.getLoginService().getRetCode() != 0) {
				...
				aUserSession.setStoreId(aUser.getStoreId() == null ? "" : aUser.getStoreId());
	 ...
	 * @throws Exception
	 */
	public void loginM(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
	        ...
	        String jsonStr = this.getHttpClientTools().doGet(requestUrl);
...
//			String openId = "dasdqjjdjk001";
	        log.info("20141205openId==" + openId);
	        
	        String passwd = "abcd1234";
	        User aUser = null;
	        //check user is exit
	        if (!this.getUserService().checkIsExist(openId)) {
	        	
		        aUser = new User();
				aUser.setNo(openId);
				aUser.setName(openId);
				aUser.setStatus(1);
				...
				if ((retVal = this.getUserService().addUser(aUser)) != 0){
					out.write("创建新用户失败!"+openId);
				}
	        }
	        

			aUser = this.getLoginService().checkLogin(openId, passwd);
			log.info("返回码=[" + this.getLoginService().getRetCode() + "] 返回信息=[" + resource.srcStr(this.getLoginService().getRetMsg()) + "]");
			if (this.getLoginService().getRetCode() != 0) {
				String retMsg = this.getLoginService().getRetMsg();
				String message = resource.srcStr(retMsg);
				...
				if (menuList.size() == 0) {
					out.write(resource.srcStr("src.userNotLogin"));
				}
				UserSession aUserSession = new UserSession();
				aUserSession.setAccount(aUser.getNo());
				aUserSession.setName(aUser.getName());
				...
				//response.sendRedirect("/wechat_app/demo3/index.html");
			}
